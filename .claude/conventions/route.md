...
- **Never write SQL or `session.exec(select(...))` in a route.** Use a CRUD function. If one doesn't exist, add it following `crud.md`.
...
- Heavy or retryable (LLM call, large doc transform, anything with timeouts) → Celery task in `app/celery/tasks/`, following `celery.md`.
