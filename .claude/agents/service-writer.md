...
You write business-logic services for kaapi-backend. Services live in `app/services/<domain>/` (auth, collections, doctransform, llm, evaluations, response, ...). Services are where orchestration happens — they call CRUD for DB work and call external HTTP libraries for third-party APIs.
