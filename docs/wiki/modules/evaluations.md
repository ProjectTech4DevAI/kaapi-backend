...
- Completion webhook (`callback_url`): `update_evaluation_run` (`crud/evaluations/core.py`) fires on the same terminal-transition guard as the completion email — when the run enters `completed`/`failed` and `callback_url` is set, `_enqueue_eval_completion_callback` runs Celery task `send_eval_completion_callback` → `services/notifications/eval_completion.py::execute_eval_completion_callback`, which POSTs `APIResponse[EvaluationRunPublic]` (success on completed, failure with `error_message` on failed) via `send_callback` (HMAC-signed with `get_webhook_secret`). Best-effort, at-least-once; only v2 runs set `callback_url`.
