...
- `crud/assessment/{core,cron,processing,batch}.py` — legacy RUN pipeline crud. RUN runtime for the dropped columns (`stage`/`stage_status`/`pipeline`/`stage_batches`/`prefilter_total_*`/`object_store_url`/`cost`) now lives in the `assessment_run.execution` bag via `core._read_exec`/`_write_exec`; `status` is the `AssessmentStatus` enum; the RUN input binding lives on the parent `assessment.input`. `batch.py` builds per-row prompts (`{column}` substitution from the parent `InputBinding.prompt`). `processing.py::process_run_batches` also records per-stage batch cost (`RunCost`, from `model_config.pricing.batch` via `estimate_model_cost`) into `execution.cost`, surfaced on `AssessmentRunPublic.cost`.
