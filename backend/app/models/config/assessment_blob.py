...
class InputColumn(SQLModel):
    ...
    ``strict`` (default) rejects a row where the column is absent or blank; ``false``
    lets rows omit it. Unknown keys are rejected so a misspelt flag fails at config save.
    """
...
class AssessmentConfigBlob(SQLModel):
    ...
    input_schema: dict[str, InputColumn] = Field(
        ...
        description=(
            ...
            "Mandatory and non-empty; a strict column (the default) must be present and "
            "non-blank in every row, a non-strict one may be omitted."
        ),
    )
