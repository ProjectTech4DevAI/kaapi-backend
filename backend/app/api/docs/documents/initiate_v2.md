...
1. `POST /api/v2/documents/uploads` with the filename — returns a `document_id` and an `upload_url`.
2. `PUT` the raw file bytes to `upload_url` — the body is the file itself, with no auth header, no form encoding, and no extra headers.
