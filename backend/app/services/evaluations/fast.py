...
def validate_and_start_fast_evaluation(
    *,
    ...
    # Flip to processing (and persist judge_config) before dispatching so the GET
    # endpoint reflects the correct state immediately.
    eval_run = update_evaluation_run(
        session=session,
        eval_run=eval_run,
        update=EvaluationRunUpdate(
            status="processing", judge_config=judge_config_payload
        ),
    )
...
def execute_fast_evaluation_aggregate(*, eval_run_id: int) -> None:
    ...
    with Session(engine) as session:
        ...
        try:
            ...
            # judge_config was persisted on the row at dispatch (the cron barrier
            # enqueues this task with only eval_run_id, so it can't ride a Celery
            # arg). NULL means the zero-config default judge.
            judge_config = (
                LLMCallConfig.model_validate(eval_run.judge_config)
                if eval_run.judge_config
                else None
            )
            run_fast_evaluation(
                session=session,
