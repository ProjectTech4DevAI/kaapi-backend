...
def inference(problem: str, solution: str) -> None:
        ...
        blob=ConfigBlob(
            completion=KaapiCompletionConfig(
                ...
                params=TextLLMParams(
                    model="gpt-4o-mini", instructions=instructions
                ).model_dump(),
            )
        )
    )
    ...
    print(response.response.output.content.value)


def inference_batch(input: Path | List[dict[str, str]]):
        ...
        if os.path.exists(input):
            ...
            data = [
                EvaluationInput(
                    ...
                    custom_id=str(row["CID"]),
                )
                for _, row in data.iterrows()
                if pd.notna(row["problem"])
                and pd.notna(row["solution"])
                and pd.notna(row["CID"])
            ]
        else:
...
if __name__ == "__main__":
    problem = """In case two trains accident in the railway station and another trains to go to another place .they got relay in India Odisha tripal train accident to solve this problem train accident prevention is is good Idea.""".strip()
    solution = """
    Matter. The train automatically detects if another train is coming from the using infrared waves and stop the engine when ever it detect some object in front automatic train collisions prevention system a ground breaking solution design to and hands railway safety by preventing solution between trains equipped with infrared sensors this innovative system to detects approaching trains and automatically halts the engine upon detecting any obstruction ahead by utilising sensors to EMIT signal and detect the reflection from object in front the system effectively identities incoming trains and triggers the Indian shutdown mechanisms through a relay with both trains equipped with these sensor collisions are awarded as each train stop upon detecting the othd in its path Benefit. Simplified operation requires minimal manual intervention providing a user friendly solution for railway safety Experience the next level of railway safety with the automatic train collisions prevention on system .
    """.strip()

    # inference(problem, solution)

    inference_batch(input=Path("./app/inquilab/inference_batch.xlsx"))
    # data = [
    #     {
    #         "problem": """In case two trains accident in the railway station and another trains to go to another place .they got relay in India Odisha tripal train accident to solve this problem train accident prevention is is good Idea.""".strip(),
    #         "solution": """ Matter. The train automatically detects if another train is coming from the using infrared waves and stop the engine when ever it detect some object in front automatic train collisions prevention system a ground breaking solution design to and hands railway safety by preventing solution between trains equipped with infrared sensors this innovative system to detects approaching trains and automatically halts the engine upon detecting any obstruction ahead by utilising sensors to EMIT signal and detect the reflection from object in front the system effectively identities incoming trains and triggers the Indian shutdown mechanisms through a relay with both trains equipped with these sensor collisions are awarded as each train stop upon detecting the othd in its path Benefit. Simplified operation requires minimal manual intervention providing a user friendly solution for railway safety Experience the next level of railway safety with the automatic train collisions prevention on system .""".strip()
    #     }
    # ]
