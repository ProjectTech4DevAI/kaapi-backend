...
GROUND_TRUTH_JUDGE_PROMPT: str = (
    ...
    "Before scoring, decompose the golden answer into its individual material "
    "claims — every distinct fact, number, date, named law/section/authority, "
    "condition, or verdict it states (a short answer often carries several, e.g. "
    "'Yes, per the Y Act' is two claims: the verdict and the named law). Check each "
    "claim against the generated answer as present, missing, or contradicted. Score "
    "from that checklist, not a holistic 'sounds about right' impression — a shared "
    "topic or matching gist is not sufficient on its own.\n"
    "- Judge meaning, not wording. A correct paraphrase or reordering of the SAME "
    "claims scores as high as the golden answer's own phrasing. Extra detail beyond "
    "the golden answer's claims does not raise the score, and does not excuse a "
    "missing or contradicted claim.\n"
    "- Extra detail is free only when it doesn't conflict with a claim the golden "
    "answer already makes. If the golden answer states a bound — 'the largest/only/"
    "last one', a closed count ('four processes'), a definitive yes/no — anything "
    "the answer adds beyond that bound (e.g. a further item ranked above the golden "
    "answer's 'largest') CONTRADICTS it, even if the added item is true in general.\n"
    ...
    "- 5: Every material claim is present, verbatim or paraphrased, and none is "
    "contradicted.\n"
    "- 4: Every material claim is present and correct, but one non-load-bearing "
    "detail (e.g. a citation or source name backing an otherwise-complete claim) "
    "is missing.\n"
    "- 3: Exactly one material claim is missing, incomplete, or contradicted; the "
    "rest are correct.\n"
    "- 2: Two or more material claims are missing or contradicted, or a single "
    "central claim (e.g. the yes/no verdict itself) is contradicted.\n"
    "- 1: Most material claims are missing or contradicted; at most one correct "
    "fragment remains.\n"
    ...
    "Reasoning: list which material claims were present vs. missing/contradicted; "
    "name the specific claim(s) that cost points.\n"
