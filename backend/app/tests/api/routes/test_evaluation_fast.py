...
class TestStage3OptOut:
    ...
    def test_opt_out_scores_and_classifies_unscoreable(
        ...
        # Stage 3 now runs the judge inline; give it a valid reply so the lone
        # scoreable row ("ok") judges cleanly and never lands in `unscoreable`.
        judge_openai = MagicMock()
        judge_openai.responses.create.return_value = SimpleNamespace(
            output_text='{"score": 1.0, "reasoning": "correct"}',
            output=[],
        ...
        with (
            ...
            _, score, write_items, _correctness = _stage3_score_and_trace(
                session=db,
                openai_client=judge_openai,
                eval_run=eval_run,
                langfuse=None,
