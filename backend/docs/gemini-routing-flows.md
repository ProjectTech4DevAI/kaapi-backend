...
Env: `GEMINI_DEFAULT_INFERENCE_ROUTE` (`"aistudio"` | `"gcp"`) — boot-validated by a `Literal` in `app/core/config.py`, so a garbage value fails startup and can never reach the registry.
Platform fallbacks: `GEMINI_API_KEY` (aistudio), `GCP_VERTEX_API_KEY` + `GCP_PROJECT_ID` + `GCP_VERTEX_LOCATION` + `GCP_SA_KEY` (gcp)
...
### H3 — Platform-routed `google`, env=`gcp`, BYOK gcp creds
Caller: `provider_type="google"`, `GEMINI_DEFAULT_INFERENCE_ROUTE="gcp"`, row `provider='google-gcp'` exists.
Flow: `_GOOGLE_ROUTED` hit → env → gcp. Lookup key rewritten to `google-gcp`. Row found → BYOK GCP client.

### H4 — Platform-routed `google`, env=`aistudio`, BYOK `google` creds
`_GOOGLE_ROUTED` hit → `GoogleAIProvider`. Lookup key is the tenant's `google` row (not `google-aistudio`). Row found → BYOK api_key.

### H5 — Platform-routed `google`, env=`gcp`, no BYOK → platform fallback
Flow: creds = None. `is_gcp_routed=True` so the missing-creds raise is skipped. `credentials={}` passed to `GoogleGCPProvider.create_client`, which per-field falls back to `GCP_VERTEX_*` / `GCP_*` settings.
...
`get_provider_credential` returns None, `is_gcp_routed=False` → raise
...
### E3 — Platform-routed `google`, env=`aistudio`, no `google` row
Unlike the gcp route, the aistudio route has no platform fallback path in the registry: `is_gcp_routed=False`, so the missing row raises
`ValueError("Credentials for provider 'google' not configured for this project.")`.

### E4 — Unknown/absent env route value
Only the exact string `"gcp"` selects `GoogleGCPProvider`; anything else resolves to `GoogleAIProvider`. In practice unreachable — the config `Literal["aistudio", "gcp"]` rejects other values at boot.
...
| `google` | `gcp` | `google-gcp` yes | gcp, BYOK |
| `google` | `gcp` | no | gcp, platform fallback |
| `google` | `aistudio` | `google` yes | aistudio, BYOK |
| `google` | `aistudio` | no | raise |
